...
router.post("/", async (req, res) => {
  ...
  /*
   * --------------------------------------------------
   * CALL FASTAPI AND SAVE SUCCESSFUL PREDICTIONS
   * --------------------------------------------------
   */
  ...
  try {
    for (const mlInput of mlInputs) {
      /*
       * 1. Send completed input
       *    to FastAPI.
       */
      const prediction = await getSalesPrediction(mlInput);

      /*
       * 2. Save the input that was
       *    sent to the ML model.
       */
      await MLModelInput.create(mlInput);

      /*
       * 3. Save FastAPI prediction.
       */
      const savedPrediction = await PredictedSales.create(prediction);

      /*
       * 4. Save successful prediction
  ...
  return renderPredictionPage(res, 200, {
    success: `Prediction completed for ${numberOfDays} day(s).`,
